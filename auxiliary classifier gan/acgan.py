...
class Generator(nn.Module):
    ...
    def forward(self, noise, labels):
        input = torch.mul(self.dict(labels).unsqueeze(-1).unsqueeze(-1), noise)
...
class Discriminator(nn.Module):
    def __init__(self, in_channels, num_labels):
        ...
        self.auxiliary = nn.Sequential(nn.Linear(64, self.num_labels), nn.Softmax(dim=1))
    ...
    def forward(self, input):
        ...
        classes = self.auxiliary(out.view(size, -1))
        return source, classes


criterionSource = nn.BCELoss()
criterionClass = nn.NLLLoss()
...
for epoch in range(0, 50):
    ...
    for i, data in enumerate(dataloader):
        ...
        D.zero_grad()
        realSource, realLabels = D(images)
        realSourceLoss = criterionSource(realSource, trueTensor.expand_as(realSource))
        realClassLoss = criterionClass(realLabels, labels)
        realLoss = (realSourceLoss + realClassLoss)/2
        realLoss.backward()
        ...
        fakeSource, fakeLabels = D(fakeData.detach())
        fakeLoss = (criterionSource(fakeSource, falseTensor.expand_as(fakeSource)) + criterionClass(fakeLabels, random_labels)) /2
        fakeLoss.backward()
        lossD = realLoss + fakeLoss
        optimizerD.step()
        G.zero_grad()
        fakeSource, fakeLabels = D(fakeData)
        lossG = (criterionSource(fakeSource, trueTensor.expand_as(fakeSource)) + criterionClass(fakeLabels, random_labels)) /2
        lossG.backward()
        optimizerG.step()
